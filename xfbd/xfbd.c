  ...
  if (my_strncasecmp (str, NOBACK, strlen (NOBACK)) && existfile (str))
    {
      ...
      if (im) 
        {
	  if (rootXpm) gdk_imlib_free_pixmap (rootXpm);
	  ...
	  rootXpm=gdk_imlib_move_image(im);
	  shapeMask=gdk_imlib_move_mask(im);
	  gdk_window_set_back_pixmap(GDK_ROOT_PARENT(), rootXpm, 0);
	  if (shapeMask) gdk_window_shape_combine_mask(GDK_ROOT_PARENT(),shapeMask,0,0);
      ...
      if (Pixbuf)
        {
	  if (rootXpm) gdk_pixmap_unref (rootXpm);
          if ((Pixbuf->art_pixbuf->width > (gdk_screen_width() / 2)) && 
	      (Pixbuf->art_pixbuf->height > (gdk_screen_height() / 2)))
          {
            pix_width  = gdk_screen_width();
            pix_height = gdk_screen_height();
          }
          ...
          {
            pix_width  = Pixbuf->art_pixbuf->width;
            pix_height = Pixbuf->art_pixbuf->height;
          }
...
#else
          {
            xpixmap = duppix(rootXpm, pix_width, pix_height);
            setPixmapProperty(xpixmap);
          }
#endif			    
#endif			    
