...
void list_lower(unsigned long *body)
{
  ...
  if(t!= NULL)
	...
	XLowerWindow(dpy,t->PagerView);
      if((t->desk>=0)&&(t->desk<ndesks))
	XLowerWindow(dpy,Desks[t->desk].CPagerWin);
  ...
  while(tline != NULL)
    {
      ...
      if((strlen(&tline[0])>1)&&
	 (mystrncasecmp(tline, CatString3("*", MyName, ".geometry"),Clength+10)==0))
	{
	  ...
	  if (flags & YValue) 
	    {
	      ...
	      (mystrncasecmp(tline,CatString3("*",MyName,".label"),Clength+7)==0))
	{
	  desk = 0;
	  ...
	  if((desk >= 0)&&(desk <=desk2))
	    {
	      n = 0;
	      while(isspace(tline[Clength+7+n]))n++;
	      while(!isspace(tline[Clength+7+n]))n++;
	      ...
	      (mystrncasecmp(tline, CatString3("*", MyName, ".font"),Clength+6)==0))
	{
