...
cb_find (GtkWidget *item, GtkWidget *ctree)
{
	...
	if (dlg_string (path, pattern) == DLG_RC_OK) {
		sprintf (path, "%s -e sh -c \"find %s -name '%s' -print | more ; read\" & ",
...
{
	if (top_length () == 1) {
		if (dlg_ask(_("Are you sure you want to quit ?")) == DLG_RC_OK)
			return (FALSE);
		return (TRUE);
	}
	return (FALSE);
			...
			int flags)
{
	...
	gtk_signal_connect (GTK_OBJECT(ctree), "drag_motion",
			GTK_SIGNAL_FUNC(on_drag_motion), win);
	gtk_signal_connect (GTK_OBJECT(ctree), "drag_leave",
			GTK_SIGNAL_FUNC(on_drag_leave), win);

	/* DND source */
	gtk_signal_connect (GTK_OBJECT(ctree), "drag_begin",
			GTK_SIGNAL_FUNC(on_drag_begin), win);
			...
			int flags)
{
	...
	label = gtk_label_new (_(" Starting XFTree ... "));
