	...
	if (strlen(temp))
	  {
		...
		g_tmp = g_tmp->next;
            ...
            if (!found) list = g_list_append(list, g_strdup (temp));
...
dlg_open_with (char *xap, char *defval, char *file)
{
	...
	if (list) gtk_combo_set_popdown_strings (GTK_COMBO(dl.combo), list);
	gtk_editable_select_region (GTK_EDITABLE(GTK_COMBO(dl.combo)->entry),0,-1);
	if (defval) gtk_entry_set_text (GTK_ENTRY(GTK_COMBO(dl.combo)->entry), defval);
