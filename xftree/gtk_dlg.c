...
dlg_new (char *labelval, char *defval, void *data, int type)
{
  GtkWidget *ok = NULL,
    *cancel = NULL,
    *all = NULL,
    *skip = NULL, *close = NULL, *icon = NULL, *combo = NULL, *label, *box;
  char title[DLG_MAX];
  char *longlabel = NULL;
  ...
  if (type & DLG_SKIP)
    {
      ...
      gtk_signal_connect (GTK_OBJECT (skip), "clicked",
			  GTK_SIGNAL_FUNC (on_ok), (gpointer)((long) DLG_RC_SKIP));
  ...
  if (type & DLG_ENTRY_VIEW)
  ...
  else if (type & DLG_ENTRY_EDIT)
    {
      ...
      gtk_signal_connect (GTK_OBJECT (dl.entry), "drag_data_received",
