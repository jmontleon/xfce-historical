...
mouse_values(XFMouse *s)
{
   ...
   XChangePointerControl(GDK_DISPLAY(), 1, 1, s->accel, DENOMINATOR, s->thresh);
   if (s->button) {
     map[0]=1;
     map[1]=2;
     map[2]=3; 
   ...
   XSetPointerMapping(GDK_DISPLAY(), map, 3);
}
...
create_xfmouse ()
{
  ...
  gtk_signal_connect (GTK_OBJECT (xfmouse), "destroy",
