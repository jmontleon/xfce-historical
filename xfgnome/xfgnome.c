...
gnome_set_win_hints (list_item *xfwm_window)
{
    ...
    if (!(xfwm_window->flags & STICKY))
        if (xfwm_window->workspace != current_desk)
            flags |= WIN_STATE_HID_WORKSPACE;

    if (xfwm_window->flags & TRANSIENT)
        flags |= WIN_STATE_HID_TRANSIENT;

    XChangeProperty (dpy, (Window) xfwm_window->id,  _XA_WIN_WORKSPACE, XA_CARDINAL,
                     32, PropModeReplace, (unsigned char *)&xfwm_window->workspace, 1);
...
gnome_handle_win_state (XClientMessageEvent *event, Window id, long mask)
{
    ...
    if (new_members & WIN_STATE_STICKY)
    {
        if (!(xfwm_window->flags & STICKY))
        {
            SendInfo (fd, "Stick", xfwm_window->id);
        }
    }
    ...
    if ((new_members & WIN_STATE_HIDDEN) || (new_members & WIN_STATE_MINIMIZED))
    {
        if (!(xfwm_window->flags & ICONIFIED))
        {
            SendInfo (fd, "Iconify", xfwm_window->id);
        }
    }
    else if ((change_mask & WIN_STATE_HIDDEN) || (change_mask & WIN_STATE_MINIMIZED))
    {
        if (xfwm_window->flags & ICONIFIED)
        {
            SendInfo (fd, "Iconify", xfwm_window->id);
