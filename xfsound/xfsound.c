...
{
	{ M_NEW_DESK,         N_("New Desk")         },
	{ M_ADD_WINDOW,       N_("Add Window")       },
	{ M_RAISE_WINDOW,     N_("Raise Window")     },
	{ M_LOWER_WINDOW,     N_("Lower Window")     },
	{ M_CONFIGURE_WINDOW, N_("Configure Window") },
	{ M_FOCUS_CHANGE,     N_("Focus Change")     },
	{ M_DESTROY_WINDOW,   N_("Destroy Window")   },
	{ M_ICONIFY,          N_("Iconify")          },
	{ M_DEICONIFY,        N_("De-iconify")       },
	{ M_SHADE,            N_("Shade")            },
	{ M_UNSHADE,          N_("Un-Shade")         },
	{ M_MAXIMIZE,         N_("Maximize")         },
	{ M_DEMAXIMIZE,       N_("De-maximize")      },
	{ 0,                  N_("Startup")          },
	{ 0,                  N_("Shutdown")         },
	{ 0,                  N_("Unknown")          }
};

static int private_menu_selected = -1;
static int prev = -1;
...
GtkWidget*
create_xfsound (XFCE_palette *pal)
{
  ...
  gtk_table_attach (GTK_TABLE (table1), soundfile_entry, 1, 2, 1, 2,
                    (GtkAttachOptions) GTK_EXPAND | GTK_FILL, (GtkAttachOptions) GTK_EXPAND | GTK_FILL, 5, 5);
  ...
  gtk_signal_connect_object (GTK_OBJECT (apply_button), "clicked",
				 ...
				 NULL);
  gtk_signal_connect_object (GTK_OBJECT (play_sound_checkbutton), "clicked",
				 GTK_SIGNAL_FUNC (doplay_cb),
				 NULL);
  gtk_signal_connect_object (GTK_OBJECT (external_player_button), "clicked",
				 GTK_SIGNAL_FUNC (defaultcmd_cb),
				 NULL);
  gtk_signal_connect_object (GTK_OBJECT (internal_player_button), "clicked",
				 GTK_SIGNAL_FUNC (internal_cb),
				 NULL);
  gtk_signal_connect_object (GTK_OBJECT (browse_button), "clicked",
				 GTK_SIGNAL_FUNC (browsefile_cb),
				 NULL);
  gtk_signal_connect_object (GTK_OBJECT (test_button), "clicked",
				 GTK_SIGNAL_FUNC (testfile_cb),
				 NULL);
  gtk_signal_connect (GTK_OBJECT (xfsound), "delete_event",
                          GTK_SIGNAL_FUNC (delete_event), NULL);
  gtk_signal_connect (GTK_OBJECT (xfsound), "destroy",
                      GTK_SIGNAL_FUNC (cancel_cb), NULL);

  return xfsound;
}

void private_update_menuentry_cb( GtkWidget *widget,
                     gpointer   data )
{
  private_menu_selected = (int) data;
  update_all_data();
}
