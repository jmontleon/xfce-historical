...
static int current_screen_selected = -1;
                   ...
                   gpointer   data )
{
  ...
  if (GTK_TOGGLE_BUTTON (widget)->active)
    show_popup_menu ((int) data, upositionx + offsetx + uwidth / 2,upositiony + offsety ,TRUE);
  else
...
gxfce_clock_show_popup_cb (GtkWidget *widget, GdkEventButton *event, gpointer data) 
{
  ...
  if (!gxfce_clock_popup_menu) gxfce_clock_make_popup (clock);
