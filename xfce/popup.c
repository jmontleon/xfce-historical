...
GtkWidget *
create_popup_item (GtkWidget * toplevel, XFCE_palette * pal, gint nbr_menu,
		   gint nbr_item)
{
  GtkWidget *hbox;
  ...
  gtk_drag_dest_set (popup_menus[nbr_menu].popup_buttons[nbr_item].
		     item_button, GTK_DEST_DEFAULT_ALL, popup_target_table,
		     n_popup_targets, GDK_ACTION_COPY | GDK_ACTION_MOVE);
...
create_popup_menu (XFCE_palette * pal, gint nbr)
{
  ...
  gtk_signal_connect (GTK_OBJECT (popup_menus[nbr].popup_detach), "clicked",
		      GTK_SIGNAL_FUNC (detach_cb), (gpointer) nbr);
...
show_popup_menu (gint nbr, gint x, gint y, gboolean animation)
{
  ...
  gdk_window_set_decorations (
			      ...
			      ((popup_menus[nbr].detach)
			       ? (GDK_DECOR_TITLE | GDK_DECOR_MENU) : 0));
