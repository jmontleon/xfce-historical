...
set_exticon_str(int i, char *s)
{
  if (i < NBSELECTS) {
    ...
    if ((s) && (strlen(s)))
      {
        selects[i].ext_icon = (char *) malloc ((strlen (s) + 1) * sizeof (char));
...
set_command (int no_sel, char *s)
{
  ...
  if (!strlen (s) || !my_strncasecmp (s, "None", strlen ("None")))
    ...
    {
      selects[no_sel].command = (char *) malloc ((strlen (s) + 1) * sizeof (char));
