...
setup_icon (void)
{
  ...
  for (i = 0; i < NBSELECTS; i++)
    if ((selects[i].icon_nbr >= 0) && (selects[i].icon_nbr < NB_PANEL_ICONS))
      MySetPixmapData (fd_XFCE->selects[i],
			  icon_data[selects[i].icon_nbr]);
    else {
      if ((selects[i].icon_nbr == 99) 
           && (selects[i].ext_icon) 
           && (existfile (selects[i].ext_icon)))
         MySetPixmapFile (fd_XFCE->selects[i], selects[i].ext_icon);
      else
        MySetPixmapData (fd_XFCE->selects[i], defaulticon);
    }
}
...
set_icon_nbr (int no_cmd, int icon_nbr)
{
  if (icon_nbr < NB_PANEL_ICONS) {
    ...
    MySetPixmapData (fd_XFCE->selects[no_cmd], icon_data[icon_nbr]);
  }
  else {
    selects[no_cmd].icon_nbr = 99;
    if ((selects[no_cmd].ext_icon) && (existfile (selects[no_cmd].ext_icon)))
         MySetPixmapFile (fd_XFCE->selects[no_cmd],selects[no_cmd].ext_icon);
      else
         MySetPixmapData (fd_XFCE->selects[no_cmd], defaulticon);
  }
}
