  ...
  if (event == FL_ENTER)
  ...
  else if ((event == FL_STEP) && ((cnt++ % 25) == 0))
    {
      ...
      py = ((ob->form->y + ob->y + ob->h + 30 < fl_scrh)
	    ...
	    : (ob->form->y + ob->y - 30));
      px = (((ob->form->x + ob->x) > 0) ? (ob->form->x + ob->x) :0);
      fl_show_oneliner (ob->u_vdata, px, py);
...
void 
set_press_button (FL_OBJECT * ob, FL_COLOR x, FL_COLOR y)
{
  fl_set_object_color (ob, x, y);
  fl_set_object_boxtype (ob, FL_FLAT_BOX);
  ob->u_ldata = x;
...
int 
date_post_handler (FL_OBJECT * ob, int event,
	      FL_Coord mx, FL_Coord my, int key, void *xev)
{
  static int cnt;
  FL_Coord px, py;
  ...
  if (event == FL_ENTER)
  ...
  else if ((event == FL_STEP) && ((cnt++ / 25) > 0))
    {
      ...
      px = (((ob->form->x + ob->x) > 0) ? (ob->form->x + ob->x) :0);
      top = time (NULL);
      timeday = localtime(&top);
      times = asctime(timeday);
      times[(int) strlen(times)-1] = '\0';
      fl_show_oneliner (times, px, py);
    }
  else if ((event == FL_LEAVE) || (event == FL_PUSH))
    {
      fl_set_object_automatic (ob, 0);
...
void 
set_tipdate (FL_OBJECT * ob)
{
  fl_set_object_posthandler (ob, date_post_handler);
}
