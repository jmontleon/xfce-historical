...
{
  XIconifyWindow(fl_display, ob->form->window, fl_screen);
}
...
update_screen (int no_screen)
{
  ...
  if (FVWM)
	...
	fl_set_button (fd_XFCE->screen[i],
		       (working_screen == i));
      if (COLORIZE_ROOT)
          ApplyRootColor (screen_colors[working_screen % 4], GRADIENT_ROOT);
    }
}
...
popup_cb (FL_OBJECT * ob, long data)
{
  ...
  popup_pulldown (menus[data], fd_XFCE->XFCE->x + x + w / 2, 
                  fd_XFCE->XFCE->y + y, h, fd_XFCE->XFCE->window, data);
}
...
quit_cb (FL_OBJECT * ob, long data)
{
  ...
  fl_set_mouse((FL_Coord) (fl_scrw / 2), (FL_Coord) (fl_scrh / 2));
  rep = (my_question (rxfce.quit, 2) == 1);
  if (rep)
    {
      if (FVWM)
...
detach_cb (FL_OBJECT * ob, long data)
{
  ...
  popup_pulldown (menus[data], -1, -1, 0, fd_XFCE->XFCE->window, data);
}
