...
{
  XIconifyWindow(fl_display, fd_XFCE->XFCE->window, fl_screen);
}
...
popup_cb (FL_OBJECT * ob, long data)
{
  ...
  popup_pulldown (menus[data], fd_XFCE->XFCE->x + x + w / 2, fd_XFCE->XFCE->y + y, h, data);
}
...
detach_cb (FL_OBJECT * ob, long data)
{
  ...
  popup_pulldown (menus[data], -1, -1, 0, data);
}
