...
create_form_menu (int no_menu, Pixmap icon, Pixmap mask)
{
  ...
  fdui->pullmenu = fl_bgn_form (FL_FLAT_BOX,
				rxfce.labelwidth + rxfce.iconwidth + 2 * (BORDERWIDTH + 1),
			   rxfce.iconheight * (NBMAXITEMS + 1) + 2 * (BORDERWIDTH + 1));
  fdui->botframe = obj = fl_add_box (FL_UP_BOX, 0, 0,
				     rxfce.labelwidth + rxfce.iconwidth + 2 * (BORDERWIDTH + 1),
				     rxfce.iconheight + 2 * (BORDERWIDTH + 1),
				     "");
  ...
  fl_add_box (FL_FRAME_BOX, (BORDERWIDTH + 1),
	      (BORDERWIDTH + 1),
	      rxfce.labelwidth + rxfce.iconwidth,
	      rxfce.iconheight, "");
  fdui->addicon = obj = fl_add_pixmap (FL_NORMAL_PIXMAP,
				       ...
				       rxfce.iconheight - 4, "");
  fl_set_object_color (obj, XFCE_COL8, XFCE_COL8);
  MySetPixmapData (obj, addiconpix);
  fl_set_object_boxtype (obj, FL_DOWN_BOX);
  fdui->addbutton = obj = fl_add_button (FL_NORMAL_BUTTON,
					 rxfce.iconwidth + (BORDERWIDTH + 1),
					 (BORDERWIDTH + 1) + 2,
					 rxfce.labelwidth - 2,
					 rxfce.iconheight - 4,
					 rxfce.pulldownadd);
  ...
  fdui->detach = obj = fl_add_button (FL_NORMAL_BUTTON,
				      0,
				      2 * (BORDERWIDTH + 1) + rxfce.iconheight,
				      rxfce.iconwidth + rxfce.labelwidth + 2 * (BORDERWIDTH + 1),
				      rxfce.detachheight,
				      "- - - - - - - - - - - - - - - -");
  ...
  for (i = 0; i < NBMAXITEMS; i++)
    {
      ...
      fdui->menuitem[i]->icon = obj = fl_add_pixmap (FL_NORMAL_PIXMAP,
						     (BORDERWIDTH + 1),
		     rxfce.iconheight * (i + 1) + 2 * (BORDERWIDTH + 1) + rxfce.detachheight,
						     rxfce.iconwidth,
						     rxfce.iconheight, "");
      fl_set_object_color (obj, XFCE_COL8, XFCE_COL8);
      fl_set_object_boxtype (obj, FL_DOWN_BOX);
	       ...
	       char *command, char *pixfile, char *label)
{
  ...
  if ((pixfile) && (existfile (pixfile)))
    ...
    MySetPixmapFile (menu->menuitem[no_item]->icon,
			menu->menuitem[no_item]->pixfile);
    }
  else
    {
      strcpy (menu->menuitem[no_item]->pixfile, "Default");
      MySetPixmapData (menu->menuitem[no_item]->icon, defaulticon);
    }
}
...
reorg_pulldown (FD_pullmenu * menu)
{
  ...
  for (i = 0; i < menu->nbitems; i++)
    {
      fl_set_object_position (menu->menuitem[i]->icon,
			      (BORDERWIDTH + 1),
			      rxfce.iconheight * (i + 1) + (BORDERWIDTH + 1));
  ...
  fl_set_object_position (menu->detach,
			  0,
			rxfce.iconheight * (menu->nbitems + 1) + 2 * (BORDERWIDTH + 1));
...
void 
popup_pulldown (FD_pullmenu * menu, FL_Coord px, FL_Coord py, FL_Coord ph, 
                Window parent, long data)
{
  int n;
  FL_Coord x, y, w, h;
  int place, deco;
  ...
  n = rxfce.iconheight * (menu->nbitems + 1);
  w = (FL_Coord) (rxfce.labelwidth + rxfce.iconwidth + 2 * (BORDERWIDTH + 1));
  h = (FL_Coord) (n + 2 * (BORDERWIDTH + 1));
  if (px > 0)
    x = menu->ix = (FL_Coord) (px - (rxfce.labelwidth + rxfce.iconwidth) / 2 - (BORDERWIDTH + 1));
  else
    x = menu->ix;
  if (py > 0)
  ...
  if ((!menu->isdetached) && (menu->nbitems > 0) && (DETACH_MENU))
    {
      h += (FL_Coord) rxfce.detachheight;
      y -= (FL_Coord) rxfce.detachheight;
    }
  ...
  if ((FVWM == 2) || (menu->isdetached))
    deco = FL_FULLBORDER; 
  else
    deco = FL_NOBORDER;
  fl_prepare_form_window (menu->pullmenu, place, deco, popup_title);
  if ((FVWM == 2) && !menu->isdetached)
    defdecor(fl_display, menu->pullmenu->window, MWM_DECOR_NONE);
  else if (menu->isdetached)
    defdecor(fl_display, menu->pullmenu->window, (MWM_DECOR_MENU | MWM_DECOR_TITLE));
  if ((FVWM == 2) || (menu->isdetached))
    XSetTransientForHint(fl_display, menu->pullmenu->window, parent);
