...
int OpenICEConn(void)
{
  ...
  sm_conn = SmcOpenConnection(NULL, &context, SmProtoMajor, SmProtoMinor,
			      ...
			      &callbacks, NULL, &sm_client_id, 4096, error_string_ret);
  if (  sm_conn  )
    {                          
      ...
      fcntl(IceConnectionNumber (SmcGetIceConnection(sm_conn)),F_SETFD,
            fcntl(IceConnectionNumber (SmcGetIceConnection(sm_conn)),F_GETFD,0) | FD_CLOEXEC);

      input_id = gdk_input_add (IceConnectionNumber (SmcGetIceConnection(sm_conn)),
                                GDK_INPUT_READ|GDK_INPUT_EXCEPTION,
                                (GdkInputFunction)process_ice_messages,
                                (gpointer) IceConnectionNumber (SmcGetIceConnection(sm_conn)));
    }
