...
FD_info *
create_form_info (Pixmap icon, Pixmap mask)
{
  FL_OBJECT *obj;
  ...
  fdui->info = fl_bgn_form (FL_FLAT_BOX, 270, 370);
  obj = fl_add_box (XFCE_BORDER, 0, 0, 270, 370, "");
  fl_set_object_color (obj, XFCE_COL8, XFCE_COL8);
  if ((fl_get_visual_depth () > 4) && (existfile (xfcedir)))
    {
      ...
      fl_set_object_lstyle (obj, XFTINY + FL_NORMAL_STYLE);
  ...
  else
    {
      ...
      fl_set_object_lstyle (obj, XFTIT + FL_EMBOSSED_STYLE);
      fl_set_object_lcol (obj, XFCE_COL10);
    }
  obj = fl_add_text (FL_NORMAL_TEXT, 10, 140, 250, 30, "by Olivier FOURDAN (c) 1997, '98, '99");
  ...
  fdui->OK_info = obj = fl_add_button (FL_NORMAL_BUTTON, 60, 330, 140, 30, rxfce.agreementlabel);
  ...
  fl_set_form_atclose (fdui->info, (FL_FORM_ATCLOSE) info_atclose, 0);
  fl_set_form_icon (fdui->info, icon, mask);
