...
{
  GtkWidget *dialog, *label, *button, *hbox, *pixmapwid;
  GdkPixmap *pixmap;
  GdkBitmap *mask;
  GtkStyle *style;

  void on_ok (GtkWidget *twidget)
  {
...
my_yesno_dialog (gchar * message)
{
     ...
     GtkSignalFunc signal_handlers[] = { on_yes, on_no };
     dialog = my_show_dialog (message, 2, buttons, 2, signal_handlers, NULL);
     gtk_signal_connect (GTK_OBJECT (dialog), "delete_event",
                          GTK_SIGNAL_FUNC (delete_event), NULL);
     gtk_signal_connect (GTK_OBJECT (dialog), "destroy",
                      GTK_SIGNAL_FUNC (delete_event), NULL);
