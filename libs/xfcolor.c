...
void write_style_to_gtkrc_file(FILE *f, XFCE_palette * p, int i, gboolean textured)
{
  int howbright;
...
applycolor(XFCE_palette * p, short int col_index, int r, int g, int b)
{
  ...
  if (howbright < fadeblack) {
    ...
    style->fg[GTK_STATE_INSENSITIVE].red     = shift (p->r[col_index], LO_MULT) << 7;
    style->fg[GTK_STATE_INSENSITIVE].blue    = shift (p->b[col_index], LO_MULT) << 7;
    style->fg[GTK_STATE_INSENSITIVE].green   = shift (p->g[col_index], LO_MULT) << 7;

    style->text[GTK_STATE_INSENSITIVE].red   = shift (p->r[col_index], LO_MULT) << 7;
    style->text[GTK_STATE_INSENSITIVE].blue  = shift (p->b[col_index], LO_MULT) << 7;
    style->text[GTK_STATE_INSENSITIVE].green = shift (p->g[col_index], LO_MULT) << 7;
  }
  else {
  ...
  style->base[GTK_STATE_SELECTED].blue       = shift (p->b[col_index], HI_MULT) << 8;
  style->base[GTK_STATE_SELECTED].green      = shift (p->g[col_index], HI_MULT) << 8;
