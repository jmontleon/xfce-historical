...
free_data(char **data)
{
  ...
  temp = data;
  sscanf(*temp, "%d %d %d %d", &w, &h, &ncolors, &bits);
...
void
ApplyRootColor (XFCE_palette * pal, gboolean gradient, short int col_index)
{
   int r = 0;
   int g = 0;
   int b = 0;
   ...
   {
      XSetWindowBackground (GDK_DISPLAY(), GDK_WINDOW_XWINDOW(GDK_ROOT_PARENT()), get_pixel_from_palette(pal, col_index));
      XClearWindow (GDK_DISPLAY(), GDK_WINDOW_XWINDOW(GDK_ROOT_PARENT()));
      XFlush (GDK_DISPLAY());
   }
}
