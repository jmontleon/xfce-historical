...
duppix(Pixmap pixmap, int width, int height)
{
    ...
    tmpDpy = XOpenDisplay("");
    if ((!tmpDpy) || (!width) || (!height)) 
	return 0;
    else 
      {
	XSync(GDK_DISPLAY(), False);

	copyP = XCreatePixmap(tmpDpy, GDK_WINDOW_XWINDOW(GDK_ROOT_PARENT()), width, height,
			       DefaultDepth(tmpDpy, DefaultScreen(GDK_DISPLAY())));
	XCopyArea(tmpDpy, pixmap, copyP, DefaultGC(tmpDpy, DefaultScreen(GDK_DISPLAY())),
  ...
  if (source)
    {
      ...
      h = requisition.height - (GTK_CONTAINER (widget)->border_width + 2) * 2;
      if ((w > 0) && (h > 0) && 
          ((source->art_pixbuf->width > w) || (source->art_pixbuf->height > h)))
        destination = gdk_pixbuf_scale_simple (source, w, h, ART_FILTER_NEAREST);
      else
        destination = gdk_pixbuf_scale_simple (source, source->art_pixbuf->width, 
                                         source->art_pixbuf->height,
                                         ART_FILTER_NEAREST);
      gdk_pixbuf_render_pixmap_and_mask (destination, &pixmap, mask, 
                                         destination->art_pixbuf->has_alpha);
