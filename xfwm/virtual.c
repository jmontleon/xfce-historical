...
initPanFrames ()
{
  ...
  attributes.cursor = Scr.XfwmCursors[TOP];
  Scr.PanFrameTop.win =
  ...
  attributes.cursor = Scr.XfwmCursors[LEFT];
  Scr.PanFrameLeft.win =
    XCreateWindow (dpy, Scr.Root,
		   ...
		   valuemask, &attributes);
  attributes.cursor = Scr.XfwmCursors[RIGHT];
  Scr.PanFrameRight.win =
    XCreateWindow (dpy, Scr.Root,
	      Scr.MyDisplayWidth - PAN_FRAME_THICKNESS, PAN_FRAME_THICKNESS,
		   ...
		   CopyFromParent, InputOnly, CopyFromParent,
		   valuemask, &attributes);
  attributes.cursor = Scr.XfwmCursors[BOTTOM];
...
void
scroll (XEvent * eventp, Window w, XfwmWindow * tmp_win, unsigned long context,
...
void
goto_page_func (XEvent * eventp, Window w, XfwmWindow * tmp_win,
		unsigned long context, char *action, int *Module)
{
  ...
  if (n != 2)
    {
      xfwm_msg (ERR, "goto_page_func", "GotoPage requires two arguments");
      return;
    }
  ...
  if ((val1_unit != Scr.MyDisplayWidth) ||
    ...
    {
      xfwm_msg (ERR, "goto_page_func", "GotoPage arguments should be unitless");
    }
