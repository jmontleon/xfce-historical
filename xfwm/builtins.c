		...
		unsigned long *context, int cursor, int FinishEvent)
{
  ...
  *w = eventp->xany.window;
  ...
  if (*w == Scr.Root)
    {
      *context = C_ROOT;
      XBell (dpy, Scr.screen);
      xfwm_msg (WARN, "DeferExecution", "Grab root window");
      UngrabEm ();
      return TRUE;
    }
  ...
  if ((*w != original_w) && (original_w != Scr.Root) &&
	...
	*context = C_ROOT;
	XBell (dpy, Scr.screen);
        xfwm_msg (WARN, "DeferExecution", "Unknown window");
	UngrabEm ();
	return TRUE;
      }
	  ...
	  unsigned long context, char *action, int *Module)
{
  ...
  if (tmp_win->flags & MAXIMIZED)
      ...
      if ((val1 == 0) && (val2 == 0))
	{
	  new_x      = MyDisplayX (center_x, center_y);
	  new_y      = MyDisplayY (center_x, center_y);
	  new_height = MyDisplayHeight(center_x, center_y);
	  new_width  = MyDisplayWidth(center_x, center_y);
	}
      tmp_win->flags |= MAXIMIZED;
      if (!(tmp_win->flags & ICONIFIED))
	{
	     ...
	     unsigned long context, char *action, int *Module)
{
  ...
  if ((font == NULL) ||
      ((Scr.StdFont.font =
	GetFontOrFixed (dpy, font, &Scr.StdFont.fontset)) == NULL
	      ...
	      unsigned long context, char *action, int *Module)
{
  ...
  if ((Scr.IconFont.font = GetFontOrFixed (dpy, font, &Scr.IconFont.fontset))
      == NULL && Scr.IconFont.fontset == NULL)
    {
...
void
use_bitmap_buttons (XEvent * eventp, Window w, XfwmWindow * tmp_win,
		    unsigned long context, char *action, int *Module)
{
  char *style = NULL;
  XfwmWindow *tmp;
  ...
  if ((mystrncasecmp (style, "On", 2) == 0) ||
    ...
    {
      xfwm_msg (ERR, "BitmapButtons",
		"Unknown parameter, using bitmap buttons");
      Scr.Options |= BitmapButtons;
    }
