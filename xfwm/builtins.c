        ...
        int f_x, int f_y, int f_w, int f_h)
{
  ...
  if (Scr.Options & AnimateWin) {
      ...
      delta_h = (f_h - o_h);
      for(j=0; j<11; j++) {
	  MoveOutline((Window) dpy , o_x + j * delta_x / 12,
                            o_y + j * delta_y / 12,
                            o_w + j * delta_w / 12,
                            o_h + j * delta_h / 12);
	  XSync(dpy, 0);
      ...
      MoveOutline ((Window) dpy, 0, 0, 0, 0);
      XFlush(dpy);
      MyXUngrabServer (dpy);
  }
}
	     ...
	     unsigned long context, char *action, int *Module)
{
  ...
  if ((mystrncasecmp (style, "Mouse", 6) == 0) ||
	     ...
	     unsigned long context, char *action, int *Module)
{
  ...
  if ((mystrncasecmp (style, "Off", 3) == 0) ||
	     ...
	     unsigned long context, char *action, int *Module)
{
  ...
  if ((mystrncasecmp (style, "Left",  4) == 0) ||
      (mystrncasecmp (style, "1",   1) == 0))
    Scr.iconbox = 1;
  else
  if ((mystrncasecmp (style, "Bottom",  6) == 0) ||
      (mystrncasecmp (style, "2",   1) == 0))
    Scr.iconbox = 2;
  else
  if ((mystrncasecmp (style, "Right",  5) == 0) ||
