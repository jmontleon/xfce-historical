...
FocusOn (XfwmWindow * t, int DeIconifyOnly)
{
  ...
  dx = cx / Scr.MyDisplayWidth * Scr.MyDisplayWidth;
  dy = cy / Scr.MyDisplayHeight * Scr.MyDisplayHeight;
...
void Shade(XfwmWindow * tmp_win)
{
  ...
  if ((tmp_win == Scr.Focus) && (tmp_win->Desk == Scr.CurrentDesk))
    {
	    ...
	    unsigned long context, char *action, int *Module)
{
  ...
  for (n = 0; n <= NBSCREENS; n++)
    for (t = Scr.XfwmRoot.next; t != NULL; t = t->next)
      {
	if ((t->flags & ICONIFIED) && (t->Desk == n) && !(t->flags & ICON_MOVED))
	  {
	    AutoPlace (t, True);
