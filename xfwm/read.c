...
static const char *read_system_rc_cmd = "Read system.xfwmrc";
static const char *read_sample_rc_cmd = "Read sample.xfwmrc";
  ...
  if (fd == NULL)
    {
      xfwm_msg (INFO,
		...
		piperead ? "command '%s' not run from %s" : "Skipping '%s': not in $HOME nor %s", ofilename, 
		(Home ? Home : XFCE_DIR));
  ...
  if (last_read_failed && this_read == 0)
    {
      ...
      if (last_read_failed && this_read == 0)
	{
	  xfwm_msg (INFO, "Read", "trying to read sample rc file");
	  ExecuteFunction ((char *) read_sample_rc_cmd, NULL, &Event, C_ROOT, -1);
	}
    }
  ...
  if (last_read_failed && this_read == 0)
    {
      ...
      if (this_read == 0)
	{
	  xfwm_msg (INFO, "PipeRead", "trying to read sample rc file");
	  ExecuteFunction ((char *) read_sample_rc_cmd, NULL, &Event, C_ROOT, -1);
	}
    }
