 ...
 *      ReGrabKeyWindows
 *
 * Set up or remove grabs on windows (fourdan@csi.com)
 *
 ***********************************************************************/

void
ReGrabKeyWindows (void)
{
  XfwmWindow *tmp;
  int j;
  ...
  for (tmp = Scr.XfwmRoot.next; tmp != NULL; tmp = tmp->next)
    {
      if(Scr.Options & (ClickToFocus | AutoRaiseWin))
	{
	   for (j = 0; j < 3; j++)
	     if (Scr.buttons2grab & (1 << j))
	       {
		 XGrabButton (dpy, (j + 1), 0, tmp->frame, True,
			      ButtonPressMask, GrabModeSync, GrabModeAsync, None,
			      Scr.XfwmCursors[SYS]);
		 XGrabButton (dpy, (j + 1), AnyModifier, tmp->frame, True,
			      ButtonPressMask, GrabModeSync, GrabModeAsync, None,
			      Scr.XfwmCursors[SYS]);
	       }
	}
	   ...
	   for (j = 0; j < 3; j++)
	     if (Scr.buttons2grab & (1 << j))
	       {
		 XUngrabButton (dpy, (j + 1), 0, tmp->frame);
		 XUngrabButton (dpy, (j + 1), AnyModifier, tmp->frame);
	       }
        }
    }
  MyXUngrabServer (dpy);
...
SetRCDefaults ()
{
  ...
  char *defaults[] =
  {
    ...
    "+ \"M\" Raise",
    "+ \"M\" Resize",
    "AddToFunc \"__buitlin_windowops_or_die__\"",
    ...
    "Mouse 0 3 A Stick",
    "Mouse 0 4 A Iconify",
    "Mouse 0 FS A Function __Raiseresize__",
    "Mouse 0 I A Function __Iconfunc__",
    "Mouse 1 1 A __buitlin_windowops_or_die__",
    ...
    "Mouse 3 1 A __buitlin_windowops_or_die__",
...
InternUsefulAtoms (void)
{
  ...
  kwm_current_desktop = XInternAtom(dpy, "KWM_CURRENT_DESKTOP", False);
  kwm_number_of_desktops = XInternAtom(dpy, "KWM_NUMBER_OF_DESKTOPS", False);
  kwm_active_window = XInternAtom(dpy, "KWM_ACTIVE_WINDOW", False);
  kwm_win_iconified = XInternAtom(dpy, "KWM_WIN_ICONIFIED", False);
  kwm_win_sticky = XInternAtom(dpy, "KWM_WIN_STICKY", False);
  kwm_win_maximized = XInternAtom(dpy, "KWM_WIN_MAXIMIZED", False);
  kwm_win_decoration = XInternAtom(dpy, "KWM_WIN_DECORATION", False);
  kwm_win_icon = XInternAtom(dpy, "KWM_WIN_ICON", False);
  kwm_win_desktop = XInternAtom(dpy, "KWM_WIN_DESKTOP", False);
  kwm_win_frame_geometry =
    XInternAtom(dpy, "KWM_WIN_FRAME_GEOMETRY", False);
     
  kwm_command = XInternAtom(dpy, "KWM_COMMAND", False);
  kwm_do_not_manage = XInternAtom(dpy, "KWM_DO_NOT_MANAGE", False);
  kwm_activate_window =
     XInternAtom(dpy, "KWM_ACTIVATE_WINDOW", False);

  kwm_running = XInternAtom(dpy, "KWM_RUNNING", False);
  kwm_module = XInternAtom(dpy, "KWM_MODULE", False);
  kwm_module_init = XInternAtom(dpy, "KWM_MODULE_INIT", False);
  kwm_module_initialized =
    XInternAtom(dpy, "KWM_MODULE_INITIALIZED", False);
  kwm_module_desktop_change =
    XInternAtom(dpy, "KWM_MODULE_DESKTOP_CHANGE", False);
  kwm_module_win_change = XInternAtom(dpy, "KWM_MODULE_WIN_CHANGE", False);
  kwm_module_desktop_name_change =
    XInternAtom(dpy, "KWM_MODULE_DESKTOP_NAME_CHANGE", False);
  kwm_module_desktop_number_change =
    XInternAtom(dpy, "KWM_MODULE_DESKTOP_NUMBER_CHANGE", False);
  kwm_module_win_add = XInternAtom(dpy, "KWM_MODULE_WIN_ADD", False);
  kwm_module_win_remove = XInternAtom(dpy, "KWM_MODULE_WIN_REMOVE", False);

  kwm_window_region_1 = XInternAtom(dpy, "KWM_WINDOW_REGION_1", False);
...
InitVariables (void)
{
  ...
  Scr.Focus = NULL;
  Scr.Ungrabbed = NULL;
  Scr.Options = (ClickToFocus | AnimateWin);
