 ...
 *      ReGrabKeyWindows
 *
 * Set up or remove grabs on windows (fourdan@xfce.org)
 *
 ***********************************************************************/
...
SetRCDefaults ()
{
  ...
  char *defaults[] =
  {
    "XORValue 0",
    "ModulePath $PATH",
    "ActiveColor white #A4A4A8",
    "InactiveColor black #A4A4A8",
    "TitleStyle Active Solid #C66B78",
    "TitleStyle Inactive Solid #9198A4",
    ...
    "AutoRaiseDelay 250",
    "SnapSize 10",
    "MenuColor  black #A4A4A8 white #8091A4",
    "IconFont fixed",
    "WindowFont fixed",
    "MenuFont fixed",
    "DeskTopSize 1 1",
    ...
    "Mouse 1 T A Function __Raisemove__",
    "Mouse 2 T A Function __Raisemove__",
    "Mouse 3 T A Shade",
...
InitVariables (void)
{
  ...
  Scr.gray_bitmap = (Pixmap) NULL; 
  Scr.gray_pixmap = (Pixmap) NULL; 
  Scr.light_gray_pixmap = (Pixmap) NULL;
  Scr.sticky_gray_pixmap = (Pixmap) NULL;
	...
	if (prop != NULL)
	  {
	    ...
	    Scr.CurrentDesk = *(unsigned long *) prop;
            ...
            syslog (LOG_DEBUG, __FUNCTION__">_WIN_WORKSPACE=%i\n", Scr.CurrentDesk); 
	  }
      }
	...
	if (prop != NULL)
	  {
	    ...
	    Scr.ndesks = *(unsigned long *) prop;
            ...
            syslog (LOG_DEBUG, __FUNCTION__">_WIN_WORKSPACE_COUNT=%i\n", Scr.ndesks); 
	  }
      }
  }
...
Done (int restart, char *command)
{
  ...
  if (Scr.gray_bitmap)		XFreePixmap(dpy, Scr.gray_bitmap); 
  if (Scr.gray_pixmap)		XFreePixmap(dpy, Scr.gray_pixmap);
  if (Scr.light_gray_pixmap)	XFreePixmap(dpy, Scr.light_gray_pixmap);
  if (Scr.sticky_gray_pixmap)	XFreePixmap(dpy, Scr.sticky_gray_pixmap);
