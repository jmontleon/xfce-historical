...
AutoPlace (XfwmWindow * t)
{
  ...
  if (t->flags & ICON_MOVED)
	...
	t->icon_y_loc += Scr.MyDisplayHeight;
      ...
      while(((test_y + temp_h) < (Scr.MyDisplayHeight+base_y))&&(!loc_ok))
	    {
	      test_x = base_x;
	      while(((test_x + temp_w) < (Scr.MyDisplayWidth+base_x))&&
		    (!loc_ok))
		{
		  real_x = test_x;
		  real_y = test_y;
...
GetXPMFile (XfwmWindow * tmp_win)
{
  ...
  if (XpmCreatePixmapFromData (dpy, Scr.Root, default_icon,
			       &tmp_win->iconPixmap,
...
Iconify (XfwmWindow * tmp_win, int def_x, int def_y)
{
  ...
  if ((tmp_win) && (tmp_win == Scr.Hilite) && (tmp_win->next))
    {
