...
DrawIconWindow (XfwmWindow * Tmp_win)
{
  ...
  if (Scr.Hilite == Tmp_win)
  ...
  else
    {
      ...
      if (Tmp_win->icon_w != None)
        XSetWindowBackground(dpy,Tmp_win->icon_w, GetDecor (Tmp_win, titlebar.state[Inactive].u.back));
      /* resize the icon name window */
      if (Tmp_win->icon_w != None)
	{
	  Tmp_win->icon_w_width = Tmp_win->icon_p_width;
  ...
  while ((tw != (XfwmWindow *) 0) && (loc_ok == True))
      ...
      if (tw->Desk == t->Desk)
	{
	  if ((tw->flags & ICONIFIED) &&
	      (tw->icon_w || tw->icon_pixmap_w) &&
	      (tw != t))
	    {
	      if (OVERLAP(t->icon_x_loc - Scr.iconspacing, 
	                  t->icon_y_loc - Scr.iconspacing,
                          t->icon_x_loc + t->icon_p_width + Scr.iconspacing, 
			  t->icon_y_loc + t->icon_w_height + t->icon_p_height + Scr.iconspacing,
                          tw->icon_x_loc, 
			  tw->icon_y_loc,
                          tw->icon_x_loc + tw->icon_p_width, 
			  tw->icon_y_loc + tw->icon_w_height + tw->icon_p_height))
		{
		  loc_ok = False;
		}
	    }
...
AutoPlace (XfwmWindow * t, Bool rearrange)
{
  ...
  switch (Scr.iconbox)
    ...
    case 2:
      base_x = 0;
      base_y = ((int) (Scr.MyDisplayHeight - height) /  Scr.icongrid) * Scr.icongrid;
      step_x = Scr.icongrid;
      step_y = -Scr.icongrid;
      break;
    case 3:
      base_x = ((int) (Scr.MyDisplayWidth - width) /  Scr.icongrid) * Scr.icongrid;
      base_y = 0;
      step_x = -Scr.icongrid;
      step_y = Scr.icongrid;
      break;
		  ...
		  if ((test_window->flags & ICONIFIED) &&
		      ...
		      (test_window != t))
		    {
		      ...
		      if (OVERLAP(tx - Scr.iconspacing, 
				  ...
				  real_x + width,
			          real_y + height) &&
			  !(rearrange && !(test_window->icon_arranged) && !(test_window->flags & ICON_MOVED)))
			{
			  loc_ok = False;
			}
		    }
...
{
  XGrabButton (dpy, AnyButton, 0, w, True,
               ButtonPressMask, GrabModeSync, GrabModeAsync, None,
               None);
  XGrabButton (dpy, AnyButton, AnyModifier, w, True,
               ButtonPressMask, GrabModeSync, GrabModeAsync, None,
               None);
  return;
}
...
GetXPMFile (XfwmWindow * tmp_win)
{
  ...
  static XpmColorSymbol none_color =
  {NULL, "None", (Pixel) 0};
...
Iconify (XfwmWindow * tmp_win, int def_x, int def_y)
{
  ...
  if ((tmp_win->Desk == Scr.CurrentDesk) && (tmp_win) && (tmp_win == Scr.Hilite))
    {
      if (tmp_win->next)
