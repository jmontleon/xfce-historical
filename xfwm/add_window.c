...
AddWindow (Window w)
{
  ...
  tflag = LookInList (Scr.TheList, tmp_win->name, &tmp_win->class, &value,
		      &Desk, &border_width, &forecolor, &backcolor,
		      &tmp_win->buttons);
  ...
  if ((XGetWindowProperty (dpy, tmp_win->w, _XA_XFWM_FLAGS, 0L, 1L, True,
	...
	{
	  tmp_win->flags |=
	    (*(unsigned long *) prop) & (STICKY | ICON_MOVED | SHADED);
	  XFree (prop);
	}
    }
  ...
  /* Reparenting is back :-( MWM is designed like this, and some 
   * Java (tm) application seem to mis it. Argh !
   */
  attributes.cursor = Scr.XfwmCursors[DEFAULT];
  tmp_win->Parent =
  ...
  if (tmp_win->flags & TITLE)
	...
	XCreateWindow (dpy, tmp_win->frame, tmp_win->title_x,
		       ...
		       CopyFromParent, (valuemask & ~CWBackPixel),
		       &attributes);
  ...
  if (tmp_win->icon_bitmap_file != NULL &&
      tmp_win->icon_bitmap_file != Scr.DefaultIcon)
    BroadcastName (XFCE_M_ICON_FILE, tmp_win->w, tmp_win->frame,
		   (unsigned long) tmp_win, tmp_win->icon_bitmap_file);
  BroadcastName (XFCE_M_RES_CLASS, tmp_win->w, tmp_win->frame,
		 (unsigned long) tmp_win, tmp_win->class.res_class);
  BroadcastName (XFCE_M_RES_NAME, tmp_win->w, tmp_win->frame,
