		      ...
		      GC Hilite, GC Shadow, int Top)
{
  ...
  XDrawLine (dpy, win, Hilite, x + 1, y, x + 1, h + y);

  XDrawLine (dpy, win, Shadow, w + x - 2, y + (Top ? 1 : 0), w + x - 2,
	     h + y);
...
PaintEntry (MenuRoot * mr, MenuItem * mi)
{
  ...
  if (mi == mr->first)
    {
      XDrawLine (dpy, mr->w, Scr.BlackGC, 0, 0, mr->width - 2, 0);
...
PopUpMenu (MenuRoot * menu, int x, int y)
{
  ...
  if ((Tmp_win) && (menu_on == 1) && (Context & C_LALL))
    {
      y =
	Tmp_win->frame_y + Tmp_win->boundary_width + Tmp_win->title_height +
	1;
    }
  ...
  /* clip to screen */
  if (x + menu->width > MyDisplayMaxX (x, y) - 2)
    x = MyDisplayMaxX (x, y) - menu->width - 2;
  if (x < 0)
    x = 0;

  if (y + menu->height > MyDisplayMaxY (x, y) - 2)
    {
      y = MyDisplayMaxY (x, y) - menu->height - 2;
    }
  if (y < 0)
    y = 0;
