	...
	{			/* Multi-byte string */
	  ...
	  if (XmbTextPropertyToTextList (dpy, &text_prop,
		...
		(char *) malloc (strlen ((char *) text_prop.value) + 1);
	      strcpy (Tmp_win->name, (char *) text_prop.value);
	    }
	}
      else
	{
      ...
      if (!(Tmp_win->name))
      {
        Tmp_win->name = (char *) malloc (strlen (NoName) + 1);
        strcpy (Tmp_win->name, NoName);
      }
	...
	{			/* Multi-byte string */
	  ...
	  if (XmbTextPropertyToTextList (dpy, &text_prop,
	  ...
	  else
	    {
	      Tmp_win->icon_name =
		(char *) malloc (strlen ((char *) text_prop.value) + 1);
	      strcpy (Tmp_win->icon_name, (char *) text_prop.value);
	    }
	}
      else
	{
...
HandleExpose ()
{
  ...
  if (Tmp_win)
      ...
      if ((Event.xany.window == Tmp_win->title_w))
	{
      ...
      else if ((Tmp_win->flags) & ICONIFIED)
	{
  ...
  if (cre->value_mask & CWStackMode)
    {
      ...
      cnt =
	(XFindContext (dpy, cre->above, XfwmContext, (caddr_t *) & otherwin)
	 == XCSUCCESS);
      xwc.sibling =
	(((cre->value_mask & CWSibling) && cnt) ? otherwin->
	 frame : cre->above);
      xwc.stack_mode = cre->detail;
      XConfigureWindow (dpy, Tmp_win->frame,
			cre->value_mask & (CWSibling | CWStackMode), &xwc);
      /* Netscape is braindead : It raises its window on top of its own transient windows ! */
      MyXGrabServer (dpy);
      for (t = Scr.XfwmRoot.next; t != NULL; t = t->next)
	{
	  if ((t->flags & TRANSIENT) && (t->transientfor == Tmp_win->w))
...
HandleRootMotionNotify ()
{
  if (Scr.SnapSize < MINRESISTANCE)
      ...
      else if (Event.xmotion.y_root == 0)
	{
      ...
      else if (Event.xmotion.x_root >= Scr.MyDisplayWidth - 1)
	{
	  Scr.EdgeScrollX++;
	}
      else if (Event.xmotion.y_root >= Scr.MyDisplayHeight - 1)
	{
	  Scr.EdgeScrollY++;
	}
      else
	...
	{			/* move desktop left */
	  ...
	  XWarpPointer (dpy, None, None, 0, 0, 0, 0,
			+(Scr.MyDisplayWidth - Scr.SnapSize), (0));
	  return;
	}
      if (Scr.EdgeScrollX >= (EDGEFACTOR * Scr.SnapSize))
	{			/* move desktop right */
	...
	{			/* move desktop upper */
	  ...
	  XWarpPointer (dpy, None, None, 0, 0, 0, 0, (0),
			+(Scr.MyDisplayHeight - Scr.SnapSize));
	  return;
	}
      if (Scr.EdgeScrollY >= (EDGEFACTOR * Scr.SnapSize))
	{			/* move desktop lower */
