...
HandleUnmapNotify ()
{
   ...
   * unmapped (which is the case for xfwm for IconicState).  Unfortunately,
   * we looked for the XfwmContext using that field, so try the window
   * field also.
   */
  weMustUnmap = 0;
  if (!Tmp_win)
...
HandleButtonPress ()
{
  ...
  else if ((Tmp_win) && (Event.xbutton.window == Tmp_win->frame))
    {
