...
GetContext (XfwmWindow * t, XEvent * e, Window * w)
{
  ...
  *w = e->xany.window;
  ...
  if ((e->type == ButtonPress) && (e->xbutton.subwindow != None) && (e->xbutton.subwindow == t->w))
...
HandleMapNotify ()
{
  ...
  if ((!(Tmp_win->flags & (BORDER | TITLE))) && (Tmp_win->boundary_width < 2))
    SetBorder (Tmp_win, False, False, True, Tmp_win->frame);
...
HandleButtonPress ()
{
  ...
  if (Tmp_win)
    {
      if ((Event.xbutton.window == Tmp_win->frame) || 
          (Tmp_win != Scr.Focus) || 
          (Tmp_win != Scr.LastWindowRaised))
        {
          if (Tmp_win != Scr.Focus)
            SetFocus (Tmp_win->w, Tmp_win, 1);
          if ((GetContext(Tmp_win,&Event, &PressedW) != C_WINDOW) || 
              (Scr.Options & (AutoRaiseWin | ClickToFocus)) || 
              (Tmp_win->flags & ICONIFIED) ||
              (!(Tmp_win->flags & (BORDER | TITLE))))
            {
              RaiseWindow (Tmp_win);
            }
          XSync(dpy,0);
...
HandleEnterNotify ()
{
  ...
  if(!(Scr.Options & ClickToFocus) && (!(e->focus)) &&
     ((e->x_root != old_x_root) || (e->y_root != old_y_root)) &&
     (e->mode == NotifyNormal))
    {
      old_x_root = e->x_root;
      old_y_root = e->y_root;
...
My_XNextEvent (Display * dpy, XEvent * event)
{
  ...
  if ((!alarmed) && (sm_fd >= 0) && (FD_ISSET(sm_fd, &in_fdset))) ProcessICEMsgs();
#endif
 
  /* Process any new X events, to save a recall if any new event have come arround */
  XFlush (dpy);
  if (XPending (dpy))
    {
      XNextEvent (dpy, event);
