	       ...
	       unsigned long context, char *action, int *Module)
{
  ...
  if (!(Scr.Options & ResizeOpaqueWin))
    {
      MoveOutline (tmp_win, dragx - tmp_win->bw, dragy - tmp_win->bw,
  ...
  if (!abort)
    {
      ...
      SetupFrame (tmp_win, dragx - tmp_win->bw,
		  dragy - tmp_win->bw, dragWidth, ((tmp_win->flags & SHADED) ? initHeight : dragHeight), FALSE, TRUE);
    }
  else
...
DoResize (int x_root, int y_root, XfwmWindow * tmp_win)
{
  ...
  if (action)
	...
	dragy = origy + origHeight - dragHeight;
	    ...
	    SetupFrame (tmp_win, dragx - tmp_win->bw, 
		      dragy - tmp_win->bw, dragWidth, ((tmp_win->flags & SHADED) ? initHeight : dragHeight), FALSE, FALSE);  
    }
}
  ...
  if (lastWidth || lastHeight)
    {
      ...
      if (tmp_win) DisplayPosition(tmp_win, lastx, lasty, lastWidth, lastHeight);
    }
...
void DisplayPosition(XfwmWindow *tmp_win, int x, int y, int w, int h)
{
  char strx[10],stry[10],strw[10],strh[10],strd[10];
  int dwidth,dheight;
  ...
  dheight = h - tmp_win->title_height - 2*tmp_win->boundary_width;
  dwidth = w - 2*tmp_win->boundary_width;
  ...
  offsetx=XTextWidth(Scr.StdFont.font,strx,strlen(strx));
  ...
  XDrawString (dpy, Scr.Root, Scr.HintsGC,
	       x - offsetx - 5, y + 1 + Scr.StdFont.height,
	       strx, strlen(strx));
  XDrawString (dpy, Scr.Root, Scr.HintsGC,
	       x + 1, y - 5,
	       stry, strlen(stry));
